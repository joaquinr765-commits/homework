...
function loadPlaylist() {
  const storedData = localStorage.getItem("customPlaylist");
  if (storedData !== null) {
    playlist = JSON.parse(storedData)
  }
}
...
function addSong(e) {
  ...
  const songArtist = artist.value
  const songLink = link.value
  const songMood = mood.value
